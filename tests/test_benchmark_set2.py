...
class TestBenchmarkSet2:
    ...
    def test_moving_load_on_hinge(self):
        ...
        rail_model_parts, mesh_1 = add_semi_rigid_hinge_at_x(rail_model_part, 25, 0, mesh_1)
        ...
        # get all element model parts from dictionary
        model_parts = [rail_model_parts + [rail_pad_model_part, sleeper_model_part, soil_1,
                        side_boundaries, load],
                       list(bottom_boundary_1.values())]
        ...
        # if RENEW_BENCHMARKS:
            ...
            position = np.linspace(rail_model_part.nodes[0].coordinates[0],
                                   rail_model_part.nodes[-1].coordinates[0], calculation_time_steps)
            # p = MovingLoad(a=0.005)
            # p.parameters(position, velocity, youngs_mod_beam, intertia_beam, rho, [winkler_mod_1],
            #              y_load)
            # p.solve()
            ...
            # # create animation
            create_animation("beam_on_hinged_winkler_foundation.html", (coords),
                             (vertical_displacements_rail[:,
                                                            len(initialisation_time) - 1:]))
            #
            # # create dictionary for results
