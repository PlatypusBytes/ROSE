...
n_sleepers = [250, 250]    # number of sleepers per segment
sleeper_distance = 0.6  # distance between sleepers, equal for each segment
depth_soil = [1, 1]      # depth of the soil [m] per segment

# Set soil parameters of each segment
stiffness_soils = [123e6, 123e7]
damping_soils = [123e3, 123e4]
...
wheel_distances = [-0.75, 0.75]  # wheel distances from the centre of the bogie [m]
bogie_length = 2                 # length of the bogie [m]

# set up cart configuration
bogie_distances = [-9, 9]        # bogie distances from the centre of the cart [m]
cart_length = 18                 # length of the cart [m]

# set up train configuration
cart_distances = [130]           # cart distances from the start of the track [m]

# set train parameters
mass_wheel = 1019.5         # mass of one wheel [kg]
mass_bogie = 16000          # mass of one bogie [kg]
mass_cart = 50000           # mass of one cart  [kg]

inertia_bogie = 0           # mass interia of one bogie   [?]
inertia_cart = 0            # mass interia of one cart   [?]

prim_stiffness = 1e6        # stiffness between wheels and bogie
sec_stiffness = 1e6         # stiffness between bogies and cart

prim_damping = 30e3         # damping between wheels and bogie
sec_damping = 30e3          # damping between bogies and cart

# set up velocity
velocity = 40               # constant velocity of the train [m/s]
...
n_t_ini = 1000          # number of time steps initialisation time  [-]

tot_calc_time = 1       # total time during calculation phase   [s]
n_t_calc = 5000         # number of time steps during calculation phase [-]
...
vertical_displacements_soil = np.array(
    [node.displacements[0::10, 1] for node in coupled_model.track.model_parts[3].nodes])
coords_soil = np.array([node.coordinates[0] for node in coupled_model.track.model_parts[3].nodes])
...
with open('tmp_track2.json',"w") as f:
