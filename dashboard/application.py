...
def write_geo_json(features, filename):
    ...
    geo_json = {"colours": ["#691aff", "#b81010", "#ffdb1a", "#6d1046", "#000066"],
...
def add_feature_to_geo_json(coordinates, time, mean_dyn_stiffness,std_dyn_stiffness,train_names, mean_cum_settlement, std_cum_settlement):
    ...
    for k, v in sos_data.items():
        ...
        for train in traffic_data:
            ...
            for k2, v2 in v["scenarios"].items():
                ...
                time,vertical_force_soil_scenario = app_utils.get_results_coupled_model(coupled_model, 10)
        ...
        cumulative_settlement_mean = np.array(sett.results["settlement"]['0'])[indices] * 1000 # in mm
