...
class TestTrack:
    def test_euler_stiffness_matrix_track(
        self, set_up_euler_rail, expected_euler_rail_stiffness_matrix
    ):
        ...
        for i in range(len(expected_stiffness_matrix)):
            for j in range(len(expected_stiffness_matrix[i])):
                assert rail_euler_stiffness_matrix[i, j] == pytest.approx(
                    expected_stiffness_matrix[i][j]
                )

    def test_euler_mass_matrix_track(
        self, set_up_euler_rail, expected_euler_rail_mass_matrix
    ):
        rail = set_up_euler_rail
        ...
        for i in range(len(expected_mass_matrix)):
            for j in range(len(expected_mass_matrix[i])):
                assert rail_euler_mass_matrix[i, j] == pytest.approx(
                    expected_mass_matrix[i][j]
                )

    def test_euler_damping_matrix(
        self, set_up_euler_rail, expected_euler_rail_damping_matrix
    ):
        rail = set_up_euler_rail
        rail.set_aux_mass_matrix()
        rail.set_aux_stiffness_matrix()
        ...
        for i in range(len(expected_damping_matrix)):
            for j in range(len(expected_damping_matrix[i])):
                assert rail_damping_matrix[i, j] == pytest.approx(
